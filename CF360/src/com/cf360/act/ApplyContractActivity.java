 ...
 * 
 */
public class ApplyContractActivity extends BaseActivity implements
		OnClickListener {
	private ScrollView scrollView;
	private LinearLayout layout_selectProduct;
	private TextView selectProduct;
	private EditText edt_baodanNumber;// 报单编号
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		baseSetContentView(R.layout.activity_apply_contract);
		initView();
		setData();
		// initData();
		initTopTitle();
	...
	private void initView() {
		...
		scrollView = (ScrollView) findViewById(R.id.apply_contract_product_scrollView);
		layout_selectProduct = (LinearLayout) findViewById(R.id.apply_contract_product_layout);
		selectProduct = (TextView) findViewById(R.id.apply_contract_product_name);
		edt_baodanNumber = (EditText) findViewById(R.id.apply_contract_edit_baodan_number);
		edt_insurance = (TextView) findViewById(R.id.apply_contract_edit_baodan_select_insurance);
		edt_age = (TextView) findViewById(R.id.apply_contract_edit_select_age);
		edt_date = (TextView) findViewById(R.id.apply_contract_edit_select_date);
		edt_customerName = (EditText) findViewById(R.id.apply_contract_edit_name);
		// edt_amount = (EditText)
		// findViewById(R.id.apply_contract_edit_investmoney);
		edt_userName = (EditText) findViewById(R.id.apply_contract_edit_financial_planners);
		edt_userPhone = (EditText) findViewById(R.id.apply_contract_edit_financial_planners_phone);
		edt_accepter = (EditText) findViewById(R.id.apply_contract_edit_receiver_name);
		edt_acceptPhone = (EditText) findViewById(R.id.apply_contract_edit_receiver_phone);
		edt_acceptAddress = (EditText) findViewById(R.id.apply_contract_edit_receiver_address);
		btnApply = (Button) findViewById(R.id.btn_apply_contract);

		layout_city = (RelativeLayout) findViewById(R.id.rl_contract_city);
		txt_city = (TextView) findViewById(R.id.tv_contract_city);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.apply_contract_product_layout:
			Intent intent = new Intent(this, SelectProductActivity.class);
			intent.putExtra("remark", "ApplyContractActivity");
			startActivityForResult(intent, 1000);
			break;
		case R.id.btn_apply_contract:
			String policyOrderId = edt_baodanNumber.getText().toString();
			...
			if (returnListContentBean != null
				...
				if (returnListContentBean.isFlag()) {// 是否保险产品
					...
					if (TextUtils.isEmpty(policyOrderId)) {
						Toast.makeText(ApplyContractActivity.this, "投保单编号不能为空",
								Toast.LENGTH_SHORT).show();
						edt_baodanNumber.requestFocusFromTouch();

					} else {
						if (returnListContentBean.getInsuranceLimit().size() != 0) {

							if (payLimitTime.equals("选择缴费年期")) {
								Toast.makeText(ApplyContractActivity.this,
								...
								if (returnListContentBean
									...
									if (ageCoverage.equals("选择年龄")) {
										Toast.makeText(
												ApplyContractActivity.this,
												"请选择年龄", Toast.LENGTH_SHORT)
												.show();
									} else {
										IsRight(policyOrderId, customerName,
												userName, userPhone, accepter,
									...
									IsRight(policyOrderId, customerName,
											userName, userPhone, accepter,
											acceptPhone, acceptAddress,
											deputyInsuranceName, ageCoverage,
											payLimitTime, city);
								}
					...
					IsRight(policyOrderId, customerName, userName, userPhone,
							accepter, acceptPhone, acceptAddress,
							deputyInsuranceName, ageCoverage, payLimitTime,
							city);
				}

			} else {
				Toast.makeText(ApplyContractActivity.this, "请选择产品",
						Toast.LENGTH_SHORT).show();
			}
			break;

		case R.id.apply_contract_edit_baodan_select_insurance:
			showDialog(multiChoiceItems,multiChoiceItemsDeputyInsuranceID);
			break;
		case R.id.apply_contract_edit_select_age:
			showDialogSingleAge(singleChoiceItemsAge, "选择年龄");
			break;
		case R.id.apply_contract_edit_select_date:
			showDialogSingleLimit(singleChoiceItemsLimit, "选择缴费年期");
			break;
		case R.id.rl_contract_city:
			try {
	...
	private void requestSearchCommissionratioData(final String productId,
		...
		if (payLimitTime.equals("选择缴费年期")) {
			Toast.makeText(ApplyContractActivity.this, "请选择缴费年期",
					Toast.LENGTH_SHORT).show();
		} else {
			if (returnListContentBean.getInsuranceAgecoverAge().size() != 0
					&& ageCoverage.equals("选择年龄")) {
				Toast.makeText(ApplyContractActivity.this, "请选择年龄",
						Toast.LENGTH_SHORT).show();
				 ...
				 */HtmlRequest.getSearchCommissionratio(
						ApplyContractActivity.this, productId,
	...
	private void IsRight(String policyOrderId, String customerName,
			String userName, String userPhone, String accepter,
			String acceptPhone, String acceptAddress,
			String deputyInsuranceName, String ageCoverage,
			String payLimitTime, String city) {

		if (TextUtils.isEmpty(customerName)) {
			Toast.makeText(ApplyContractActivity.this, "客户名不能为空",
					Toast.LENGTH_SHORT).show();
			edt_customerName.requestFocusFromTouch();
		} else {
			if (TextUtils.isEmpty(accepter)) {
				Toast.makeText(ApplyContractActivity.this, "收件人不能为空",
						Toast.LENGTH_SHORT).show();
				edt_accepter.requestFocusFromTouch();
			} else {
				if (TextUtils.isEmpty(acceptPhone)) {
					Toast.makeText(ApplyContractActivity.this, "收件人手机号码不能为空",
							Toast.LENGTH_SHORT).show();
						...
						if (city.equals("所在地")) {
							...
							if (TextUtils.isEmpty(acceptAddress)) {
								...
								if (PreferenceUtil.getUserType().equals("corp")) {
									...
									requestApplyContractData(productId,
											productName, productCategory,
											customerName, userName, userPhone,
											accepter, acceptPhone,
											city + acceptAddress, policyOrderId,
											deputyInsuranceName, payLimitTime,
											ageCoverage);
	...
	private void requestDataSelectProduct(final String id, String category,
			String appoId) {
		HtmlRequest.getSelectProductReturn(ApplyContractActivity.this, id,
	...
	protected void setRequestData(
		...
		edt_baodanNumber.setVisibility(View.GONE);
		edt_baodanNumber.setText("");
		...
		if (returnListContentBean.isFlag()) {// 是否保险产品
			edt_baodanNumber.setVisibility(View.VISIBLE);
		...
		multiChoiceItems = new String[childBeans.size()];
		for (int i = 0; i < childBeans.size(); i++) {
			multiChoiceItems[i] = String.valueOf(childBeans.get(i).getNAME());
		}
		defaultSelectedStatus = new boolean[multiChoiceItems.length];

	}

	private void requestApplyContractData(final String productId,
			String productName, String productCategory, String customerName,
			String userName, String userPhone, String accepter,
			String acceptPhone, String acceptAddress, String policyOrderId,
		 ...
		 * payLimitTimeone.lastIndexOf("年")); }
		 */
		HtmlRequest.getApplyContract(ApplyContractActivity.this, productId,
				productName, productCategory, customerName, userName,
				userPhone, accepter, acceptPhone, acceptAddress, policyOrderId,
				deputyInsuranceName, payLimitTimeone, ageCoverage,
				new OnRequestListener() {

					@Override
					public void onRequestFinished(BaseParams params) {
						if (params.result != null) {
							ResultApplyContractContentBean data = (ResultApplyContractContentBean) params.result;
							if (data != null) {
								if (data.getFlag().equals("true")) {
									Toast.makeText(ApplyContractActivity.this,
											"合同申请成功", Toast.LENGTH_LONG).show();
									Intent intent = new Intent(
											ApplyContractActivity.this,
											ContractActivity.class);
									ApplyContractActivity.this.setResult(2,
											intent);
									ApplyContractActivity.this.finish();
									requestSMS(data.getContractId(),
											data.getProductName());
								} else {
									Toast.makeText(ApplyContractActivity.this,
											data.getMessage(),
	...
	private void requestSMS(String contractId, String productName) {
		try {
			String mobile = DESUtil.decrypt(PreferenceUtil.getPhone()
					.toString());
			HtmlRequest.sentSMSForApplyContract(ApplyContractActivity.this,
					mobile, ApplicationConsts.APPLY_CONTRACT, contractId,
	...
	private void initPopWindowForCitys() {
		LayoutInflater inflater = LayoutInflater
				.from(ApplyContractActivity.this);
		...
		ApplyContractActivity.this.getWindowManager().getDefaultDisplay()
				.getMetrics(dm);
		...
		// }
		// });
		WindowManager.LayoutParams params = ApplyContractActivity.this
				.getWindow().getAttributes();
		params.alpha = 0.7f;

		ApplyContractActivity.this.getWindow().setAttributes(params);
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (citypopupWindow.isShowing()) {
					...
					WindowManager.LayoutParams params = ApplyContractActivity.this
							.getWindow().getAttributes();
					params.alpha = 1f;
					ApplyContractActivity.this.getWindow()
