...
import android.support.v4.myview.VerticalViewPagerCompat;
...
public class DirectionalViewPager extends ViewPager {
	...
	protected boolean canScroll(View v, boolean checkV, int dx, int x, int y) {
		// DetailWebView webview = (DetailWebView) v
		// .findViewById(R.id.prodetail_web);
		// if (webview != null) {
		// return webview.canScrollVertically(-dx);
		// } else {
		// return super.canScroll(v, checkV, dx, x, y);
		// }
		if (v instanceof DetailWebView) {
			return v.canScrollVertically(dx);
		} else if (v instanceof DetailScrollView) {
			return v.canScrollVertically(dx);
		} else {
			return super.canScroll(v, checkV, dx, x, y);
		}
